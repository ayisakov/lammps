...
c fixed boundary conditions - reflect atom from the wall of the box
      subroutine fbc(x,y,z)
      common xprd,yprd,zprd,xboundlo,xboundhi,
     $     yboundlo,yboundhi,zboundlo,zboundhi

      if (x < xboundlo) x = x + 2*(xboundlo - x)
c     TODO: handle the case of the coordinate being more than one box
c     dimension away from the boundary (e.g. while (xboundlo - x) > xprd: x = x - xprd)
      if (x >= xboundhi) x = x - 2*(x - xboundhi)
      if (y < yboundlo) y = y + 2*(yboundlo - y)
      if (y >= yboundhi) y = y - 2*(y - yboundhi)
      if (z < zboundlo) z = z + 2*(zboundlo - z)
      if (z >= zboundhi) z = z - 2*(z - zboundhi)
